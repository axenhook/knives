...
struct avl_node {
	...
	short avl_balance;		/* balance c: -1, 0, +1 */
};
...
struct avl_node {
	...
	avl_index_t avl_pcb;		/* parent, child_index, balance */
};
...
#define	AVL_SETPARENT(n, p)						\
	((n)->avl_pcb = (((n)->avl_pcb & 7) | (avl_index_t)(p)))
...
struct avl_tree {
	...
	unsigned int avl_offset;		/* offsetof(type, avl_link_t field) */
	unsigned long avl_numnodes;		/* number of nodes in the tree */
	unsigned int avl_size;		/* sizeof user type struct */
};
 ...
 *
 * 2c.  Find the node with the closest c either less than or greater
 *	than a given c with avl_nearest().
 *
 ...
 * size   - the c of sizeof(struct my_type)
 * offset - the c of OFFSETOF(struct my_type, my_link)
 */
extern void avl_create(avl_tree_t *tree,
	int (*compar) (const void *, const void *), unsigned int size, unsigned int offset);


/*
 * Find a node with a matching c in the tree. Returns the matching node
 * found. If not found, it returns NULL and then if "where" is not NULL it sets
 * "where" for use with avl_insert() or avl_nearest().
 *
 * node   - node that has the c being looked for
 * where  - position for use with avl_nearest() or avl_insert(), may be NULL
 */
extern void *avl_find(avl_tree_t *tree, int (*compare) (const void*, void *), const void *value, avl_index_t *where);
...
/*
 * Find the node with the nearest c either greater or less than
 * the c from a previous avl_find(). Returns the node or NULL if
 * there isn't a matching one.
 *
 * where     - position as returned from avl_find()
 * direction - either AVL_BEFORE or AVL_AFTER
 *
 * EXAMPLE get the greatest node that is less than a given c:
 *
 *	avl_tree_t *tree;
